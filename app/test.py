...
CHOOSING, TYPING_REPLY, TYPING_CHOICE, MAIN_MENU, ORDER_STATUS, ORDER_STATUS_PROGRAM,DISTRICT_REPORTING_RATE_PROGRAM, DISTRICT_REPORTING_RATE, COMING_SOON, SELF_CARE, DESCRIPTION, CONFIRM, VIEW_TICKET_BY_KEY, SEARCH_TICKET_BY_KEY, ACCESS_ACC_STEPS, ACCESS_ACC_STEPS_TWO, CLEAR_CACHE_STEPS, CLEAR_CACHE_STEPS_TWO , HELP, FILL_LOGIN_CREDENTIALS, SIGN_IN, CLEAR_CACHE, ADD_PRODUCT, ADD_PRODUCT_STEP_TWO, GET_PROGRAME_NAME, GET_MSD_CODE, MIN_REPORT, START, RNR_STATUS= range(29)
...
def start(update, context):
    welcome_html_string = ('<b>Hello ' + update.message.chat.first_name.upper() + ' ' + update.message.chat.last_name.upper() + '!</b> \n'
...
def main_menu(update, context):
    ...
    welcome_html_string = ('<b>Hello ' + query.message.chat.first_name.upper() + ' ' + query.message.chat.last_name.upper() + '!</b> \n'                                                                                                                        'Choose between below options')
...
def help(update, context):
    ...
    query.message.reply_text("Hello! Choose",reply_markup=reply_markup)
    return HELP
...
def add_product_main(update, context):
    ...
    query.message.reply_text("All the items that are available in the msd catalog have been registered in the eLMIS. You should also remember that, some items are found in priority drugs and medical supply template while other others are in the additional"
                             " drugs and medical supply template. So, what you have to do first is to check in all  So, what you have to do first is to check in all pages of your R&R available in the eLMIS, and if you can’t find it and the item is found in "
...
def mini_reports(update, context):
    ...
    query.message.reply_text("Hello! Choose",reply_markup=InlineKeyboardMarkup(reply_inline))
    return RNR_STATUS
...
def program_name(update, context):
    ...
    reply_keyboard = [['ILS', 'ARV', 'Lab System', 'TB']]
    markup = ReplyKeyboardMarkup(reply_keyboard, one_time_keyboard=True)
    update.message.reply_text("Choose program",reply_markup=markup)
    return GET_PROGRAME_NAME


def get_msd_code(update, context):
    query = update.callback_query
    query.message.reply_text("What is the MSD code of your facility")
    return GET_MSD_CODE
...
def getrnrstatusbyfacilitycode(update, context):
    ...
    program = getprogramcode(user_data['program'])
    ...
    if "error" in data:
        reply_markup = InlineKeyboardMarkup(main_menu_keyboard)
        update.message.reply_text(data["error"] + ", Go back to main menu",reply_markup=reply_markup)
        return MAIN_MENU
    ...
    reply_keyboard = [['Subscribe', 'Go back to Main Menu']]
    markup = ReplyKeyboardMarkup(reply_keyboard, one_time_keyboard=True)
    update.message.reply_text("Choose to subscribe for this RNR and get notification whenever status change or go back to main menu",reply_markup=markup)
    return CHOOSING
...
def subscribe(update, context):
    ...
    params = {'chatId' : update.message.chat_id, 'rnrId' : rnrid, 'label': 'REQUISITION_STATUS'}
    response = requests.get(API_SUBSCRIBE, params = params)

    reply_markup = InlineKeyboardMarkup(main_menu_keyboard)
    update.message.reply_text("You have successfully subscribed, you will receive notification whenever status change",reply_markup=reply_markup)
    return MAIN_MENU
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            GET_PROGRAME_NAME:[MessageHandler(Filters.text,
